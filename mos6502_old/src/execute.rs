...
fn group_one_aaa(aaa: u8) -> impl Fn(u8, u8, u8) -> u8 {
    ...
    fn group_one_bbb(bbb: u8, mos: &mut Mos) -> u8{
        match bbb{
            3 =>{
                // TODO Need to set separate function to read_pc twice
                // absolute
                mos.memory[mos.read_pc() as usize] // Return the value in that exact memory location
            }
            1 => {
                // Zero Page
                mos.memory[mos.read_pc() as usize]
            }

            _ => unimplemented!("Unknown bbb for group one {}", bbb)
        }
    ...
    pub fn group_one(mos: &mut Mos, aaa: u8, bbb: u8, cc: u8){
        ...
        // Addressing, return the u8 value specified
